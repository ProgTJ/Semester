...
static int shoulderAngle = 0, elbowAngle = 0, fingerAngle = 0, thumbAngle = 0;

// Handles the keyboard event: the left and right arrows bend the elbow, the
// up and down keys bend the shoulder.
void special(int key, int, int) {
  switch (key) {
    case GLUT_KEY_LEFT: if (elbowAngle<120)(elbowAngle += 5) %= 360; break;
    case GLUT_KEY_RIGHT: if (elbowAngle > 0)(elbowAngle -= 5) %= 360; break;
    case GLUT_KEY_UP: (shoulderAngle += 5) %= 360; break;
    case GLUT_KEY_DOWN: (shoulderAngle -= 5) %= 360; break;
    case GLUT_KEY_PAGE_UP: if (fingerAngle < 90)(fingerAngle += 5) %= 360; break;
    case GLUT_KEY_PAGE_DOWN: if (fingerAngle > 0)(fingerAngle -= 5) %= 360; break;
    case GLUT_KEY_INSERT: if (thumbAngle > -90)(thumbAngle -= 5) %= 360; break;
    case GLUT_KEY_HOME: if (thumbAngle < 0)(thumbAngle += 5) %= 360; break;
    default: return;
  }
  glutPostRedisplay();
}
...
void display() {
  ...
  glColor3f(1.0,1.0,1.0);
  glTranslatef(1.25, -0.15, 0.0);
  wireBox(0.5, 0.1, 1.0);
  
  glPushMatrix();
  	glColor3f(1.0,1.0,1.0);
  	glTranslatef(0.0, 0.0, 0.5);
  	glRotatef((GLfloat)thumbAngle, 1.0, 0.0, 0.0);
